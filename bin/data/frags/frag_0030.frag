...
//						|/
// By       Dave Hoskins
// Edited in the new shadertoy:  https://www.shadertoy.com/view/4sl3RH
// But runs twice as fast here, for some reason!
...
const vec3 areaColor = vec3(.53, .7, .85);
...
const vec3 ballStart1 = vec3(-.0, 1.6, -1.);
...
const vec3  QbertStart = vec3(-3.0, -1.4, .00);
...
float GetQbertLocation(float s, out vec3 out1, out vec3 out2)
{
	...
	for (int t = 0; t < 19; t++)
	{
		...
		//float n = rand(float(t+4163));
		float n = rand(float(t+5920));
		if (n >= .75)
		{
			//if (pos.x < .5)
			{
...
vec3 LightCubes(vec3 pos, vec3 n, vec3 eyePos)
{
...
float DistanceFields(vec3 ro, vec3 rd, out int hit, out vec3 pos)
{
	...
	for (int st = 0; st < 22; st++)
	{
		if (hit == 0)
		{
			...
			{
				hit = m+1;
				//break; // Why can't Shadertoy handle 'break' or 'return' in loops?
			}
			len += d;
		}
	}
...
void main(void)
{
	...
	if (material > 0 && material != 6)
	{
		...
		rgb = mix(rgb, DoMaterialRGB(material, pos, norm, rd, ro), .13);
	}
#endif	
	
	// Curve the brightness a little...
	//rgb = pow(rgb, vec3(.7, .7, .7));
