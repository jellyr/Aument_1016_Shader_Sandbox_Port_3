...
void main( void ) {
	...
	for (int i = 0; i < 15; ++i) {
		vec2 center = resolution/2.0 + vec2(sin(0.5 * float(random()) *time * 5.0 + pow(2.0, float(i*2)))*400.0, cos(0.15 * (float(i)-2.0) * time) * 200.0);
		float dist = length(pixel_pos-center);
		float intensity = pow((8.0 + 20.0 * mod(float(i), 2.5))/dist, 3.0);
